function [images, n_inclusions]= randomSheppLogan(n,param)
%
...
for m = 1:M
  ...
  if default % use standard Shepp-Logan phantom
    ...
    [rphantom, n_inclusions(m)] = modify(phantom); % get random variation of phantom
  end
...
function [phantom, n_inclusions] = modify(phantom)
...
% random rotation
rotation = 2*60*(rand-0.5); phantom(:,6) = rotation + phantom(:,6);

% random translation
translate = 0.6*(rand(1,2)-0.5); phantom(:,4:5) = repmat(translate,m,1) + phantom(:,4:5);
...
obj = 6; idx = randsample(m-obj,randi(m-obj)-1,'false'); phantom(idx+obj,:) = [];
n_inclusions = length(idx);
...
elseif strcmp(type,'msl')
  ...
  phantom = ...
    [  1    0.69    0.92      0        0    0   % outer skull cap boundaries
     ...
     0.025  0.1100  0.3100   0.22        0  -18   % ventricle left (intensity corrected since not adding)
     0.025  0.1600  0.4100  -0.22        0   18   % ventricle right (intensity corrected since not adding)
